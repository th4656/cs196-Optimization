...
public class AnalyzeText {
	public static TreeMap<String, String[]> nutritionDetails(String s) {
		...
		TreeMap<String, String[]> t = new TreeMap<String, String[]>(); // Creating
																		// treemap
																		// that
																		// will
																		// hold
		...
		while (!TextIO.eof()) { // loop through the text file.
			...
			ArrayList<String> quantity = new ArrayList<String>(); // array list
																	// to hold
																	// quantities

			String tempLine = TextIO.getln(); // read in the current line of the
												// text file

			Pattern percentQuantities = Pattern.compile("\\d+%"); // regex to
																	// search
																	// for all
																	// numbers
																	// followed
			...
			Pattern gramsQuantity = Pattern.compile("\\d+(g|mg)"); // regex to
																	// search
																	// for all
																	// numbers
																	// followed
			...
			Pattern calorieCounter = Pattern.compile("Cal.*\\s\\d+"); // regex
																		// to
																		// search
																		// for
																		// calories
			...
			while (percentMatcher.find()) { // add the values found by regex
											// into the array list
			...
			String[] quantityArray = new String[quantity.size()]; // convert
																	// array
																	// list to
																	// an array
			...
			/**
			 * fuzzy spelling search goes here to put values into data and
			 * amount.
			 */
			...
			if (Fuzzy.containsOneOf(tempLine, "Carbs")
					|| Fuzzy.containsOneOf(tempLine, "Carbohydrates")) {
				data = "Carbs";
			}
	 ...
	 */
	public int dailyCalories(String gender, double exerciseAmount,
	...
	public int dailyProteinLow(int bmr, int age) {
		if (age < 4) {
			bmr = (int) (bmr * 0.2);
		} else if (age < 19) {
			bmr = (int) (bmr * 0.3);
	...
	public int dailyProteinHigh(int bmr, int age) {
		if (age < 4) {
			bmr = (int) (bmr * 0.2);
		} else if (age < 19) {
			bmr = (int) (bmr * 0.3);
	...
	public int dailyCarbs(int bmr, int age) {
		bmr = (int) (bmr * 0.45);
		bmr = bmr/4;
		return bmr;
	}
	public int dailyFatLow(int bmr, int age) {
		if (age < 4) {
			bmr = (int) (bmr * 0.3);
		} else if (age < 19) {
			bmr = (int) (bmr * 0.35);
	...
	public int dailyFatHigh(int bmr, int age) {
		if (age < 4) {
			bmr = (int) (bmr * 0.4);
		} else if (age < 19) {
			bmr = (int) (bmr * 0.35);
	...
	public static void main(String[] args) {
		TreeMap<String, String[]> tree = AnalyzeText
				.nutritionDetails("test.txt");
